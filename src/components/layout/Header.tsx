...
// Safely lazy-load SVGs and internal sub-components on client-side only to bypass SSR failures
const MenuIcon = dynamic(() => import('@/assets/icons/menu.svg'), { ssr: false });
const XIcon = dynamic(() => import('@/assets/icons/x.svg'), { ssr: false });
const Logo = dynamic(() => import('@/assets/icons/logo.svg'), { ssr: false });
const Btn = dynamic(() => import('../Btn'), { ssr: false });
...
const Header: React.FC = () => {
    ...
    return (
        <header className="relative z-50 w-full" ref={navItemsDOM}>
            <div className="relative z-10 container mx-auto px-4">
                <nav className="relative flex items-center justify-between pt-6 lg:pt-8">
                    ...
                    <Link href="/" className="flex md:flex-1" aria-label="Home page">
                        <div className={classNames('w-[60px] md:w-[101px]', {
                            'max-md:grayscale-0 max-md:brightness-[0.2] max-md:invert-0': showMobileMenu,
                        })}>
                            <Logo />
                        </div>
                    </Link>
                    ...
                    <ul className={classNames(
                        ...
                        {nav.map((item, index) => (
                            <li key={index} className="text-center md:text-left">
                                <Link
                                    ...
                                    className="text-lg font-medium leading-none tracking-[-0.41px] uppercase transition-colors duration-200 hover:text-appAccent md:text-sm"
                                >
                                    {item.label}
                                </Link>
                            </li>
                        ...
                        <li className="text-center md:hidden">
                            <Link
                                href="/contact"
                                onClick={() => setShowMobileMenu(false)}
                                className="text-lg font-medium leading-none tracking-[-0.41px] uppercase"
                            >
                                Contact us
                            </Link>
                        </li>
                    ...
                    <button
                        className="flex p-2 focus:outline-none md:hidden"
            ...
            {showMobileMenu && (
                <div className="fixed inset-0 z-0 w-screen h-screen bg-appAccent md:hidden" />
            )}
        </header>
    );
};
