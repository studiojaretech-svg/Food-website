...
const FormText: React.FC<FormTextProps> = ({
    ...
    return (
        <label className={classnames('relative flex flex-col', className)}>
            {/* Restored the exact original horizontal/vertical alignment positions of the labels */}
            {label && (
                <div className="absolute -top-2 left-[14px] px-[14px] bg-[#D7BDA6] text-[11px] font-bold tracking-wider leading-none text-[#4C2B08] uppercase lg:left-7 lg:text-sm lg:leading-none">
                    {label}
                </div>
            )}
            {type === 'textarea' ? (
